...
"""
import torch
from utils.core_nns_sln import UniLSTMModel, BiLSTMModel
...
class SentiInference:
    ...
    def predict(self, doc="", topk=5):
        ...
        doc_tensor = Data2tensor.idx2tensor(doc_ids, self.device).reshape(1, -1)
        doc_lengths_tensor = Data2tensor.idx2tensor([doc_tensor.shape[1]], self.device)
        self.model.eval()
        with torch.no_grad():
            score, _, _ = self.model(doc_tensor, doc_lengths_tensor)
            pred_prob, pred_idx = self.model.inference(score, k=min(topk, len(self.args.vocab.l2i)))
            pred_lb = [self.args.vocab.i2l[tok_id.item()] for tok_id in pred_idx.squeeze()]
            pred_probs = pred_prob.squeeze().tolist()
