    ...
    def evaluate_batch(self, eval_data):
        ...
        with torch.no_grad():
            for doc_batch, lb_batch in eval_batch:
                ...
                batch_loss = self.model.NLL_loss(output, lb_tensor)
                total_loss += batch_loss.item()

                label_prob, label_pred = self.model.inference(output, k=1)
                y_pred += label_pred.squeeze().tolist()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--hidden_size', type=int, default=100, help='number of hidden units per layer')
